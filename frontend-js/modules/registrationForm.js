...
export default class RegistrationForm {
    ...
    formSubmitHandler(){
        this.usernameInmediately()
        this.usernameAfterDelay()
        this.emailAfterDelay()
        this.passwordInmediately()
    ...
    isDifferent(element, handler){
        ...
        element.previousValue = element.value //Updating the previous value
    }
    
    usernameHandler(){
        this.username.errors = false // no errors property
        this.usernameInmediately()
    ...
    passwordHandler(){
        this.password.errors = false // no errors property
        this.passwordInmediately()
    ...
    emailHandler(){
        this.email.errors = false // no errors property
        ...
        if(!this.email.errors){
            axios.post('doesEmailExist', { _csrf: this._csrf , email: this.email.value})
                 .then((response)=>{
                    if(response.data){
                        this.email.isUnique = false
                        this.showValidationError(this.email, "This email address is already being used.")
                    }else{
    ...
    showValidationError(element, message){
        ...
        element.errors = true // property with errors
    }
